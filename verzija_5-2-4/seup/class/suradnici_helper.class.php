...
{
    /**
     * Ensure llx_a_posiljatelji table exists
     */
    public static function ensurePosiljareljiTable($db)
    {
        $table_name = MAIN_DB_PREFIX . "a_posiljatelji";
        ...
        if ($resql && $db->num_rows($resql) == 0) {
            ...
            $result = $db->query($sql);
            if (!$result) {
                dol_syslog("Error creating a_posiljatelji table: " . $db->lasterror(), LOG_ERR);
                return false;
            }
            dol_syslog("Table a_posiljatelji created successfully", LOG_INFO);
        }
        return true;
    }
